...
function getMetadata(data: any) {
  return formatMetadata({
    ...
    alternates: {
      canonical:
        data?.canonical_link || pageUrl(BlogTypes.BUSINESS_LOUNGE, true),
    },
    openGraph: {
      images: [
        {
...
export default async function ProductBusinessLounge() {
  const optionsFilter = (await ProductBusinessLoungeApi.getOptionsFilter())
    ?.payload?.data as any;
        ...
        <div className="mt-[68px] px-3 lg:mt-0 lg:pt-[132px] lg:px-[80px] max__screen">
          <Breadcrumb className="pt-3">
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <p className="text-gray-700">{t("business_lounge")}</p>
                </BreadcrumbLink>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
