...
export default function SimDuLichHeroFilters() {
  ...
  const handleSearch = () => {
    if (selectedDestinationOption) {
      const href = resolveDefaultSimDuLichPackageHref(
        { value: selectedDestinationOption.value ?? "", label: selectedDestinationOption.displayLabel },
        packages,
        activeLocale
      );
      if (href) {
  ...
  return (
    <div className="mt-4 flex flex-col md:flex-row md:items-center gap-3 w-full">
      ...
      <div className="w-full md:w-3/12 shrink-0">
        <button
          type="button"
          onClick={handleSearch}
          className="w-full h-[52px] rounded-[14px] bg-orange-700 hover:bg-orange-800 transition-colors text-white font-medium flex items-center justify-center gap-2"
        >
          <svg
            className="w-5 h-5"
            fill="none"
