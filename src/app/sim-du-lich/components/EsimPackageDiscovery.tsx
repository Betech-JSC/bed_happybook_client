...
import { ChevronDown, Clock3, Globe, MapPin, Search, SlidersHorizontal, Wifi, X } from "lucide-react";
...
export default function EsimPackageDiscovery({
  ...
  const filterSheetOptions = (items: DestinationSelectOption[]) => {
    const keyword = normalizeText(mobileSearch);
    if (!keyword) return items;

    return items.filter((option) =>
      normalizeText([option.displayLabel, option.label, option.value].filter(Boolean).join(" ")).includes(keyword)
    );
  };
  ...
  const renderMobileSheet = () => {
    ...
    const title = mobileSheet === "location" ? t("Location", "Location") : t("Filters", "Filters");
      ...
      <div className="fixed inset-0 z-[80] lg:hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 max-h-[88vh] overflow-hidden rounded-t-[22px] bg-white shadow-2xl">
          ...
          <div className="flex h-14 items-center justify-center border-b border-slate-100 px-4">
            ...
            <h3 className="text-base font-bold text-midnight-ink">{title}</h3>
          </div>
              ...
              <div className="space-y-6">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t("Tìm theo thành phố, khu vực hoặc quốc gia", "Search by city, region or country")}
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-4 border-t border-slate-100 bg-white px-5 py-4">
            ...
            >
              {t("Show", "Show")} {packages.length} {t("kết quả", "results")}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="pt-8">
      {showInternationalFilters ? (
        <>
          ...
          <div className="hidden lg:block lg:sticky lg:top-32 h-fit">{renderSidebar()}</div>
          ...
          <div>
            ...
            <div className="mb-3 flex items-center justify-between text-sm text-midnight-ink lg:hidden">
              <span>{packages.length} {t("kết quả", "results")}</span>
              <button
                type="button"
                onClick={() => openMobileSheet("filters")}
