    ...
    <div className="lg:col-span-4 lg:sticky lg:top-[140px] space-y-6 h-fit">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-100/50 overflow-hidden">
        <div className="p-6 md:p-8 space-y-6">
          <h3 className="text-xl font-bold text-midnight-ink">{t("Tóm tắt đơn hàng")}</h3>
          ...
          <div className="flex gap-4 pb-6 border-b border-slate-100">
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-slate-800 leading-tight">eSIM {activePackageLabel}</h4>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-slate-600 font-medium">
              ...
              <span className={serviceFee === 0 ? "text-slate-400 italic font-normal" : ""}>
                {serviceFee === 0 ? t("Miễn phí") : formatCheckoutAmount(serviceFee, currency)}
              </span>
            </div>
            {paymentMethod === "onepay" && (checkoutData?.payment_fee_amount ?? 0) > 0 ? (
              <div className="flex justify-between text-slate-600 font-medium">
                <span>{t("Phí thanh toán OnePay")}</span>
                <span>
                  {formatCheckoutAmount(checkoutData?.payment_fee_amount ?? 0, checkoutData?.currency || currency)}
                </span>
              </div>
            ) : null}
          </div>
          ...
          <div className="pt-6 border-t border-slate-100 space-y-6">
            <div className="flex flex-col items-end">
              <span className="text-sm font-bold text-slate-500 mb-1">{t("Số tiền thanh toán:")}</span>
            ...
            {quoteError ? <p className="text-sm text-red-500">{quoteError}</p> : null}

            {!quoteIsAvailable ? (
              <p className="text-sm text-red-500">
                {t("Gói eSIM này hiện chưa thể thanh toán. Vui lòng chọn gói khác.")}
              </p>
            ) : null}

            {orderCode ? <p className="text-xs text-slate-400 text-right">{t("Mã đơn hàng:")} {orderCode}</p> : null}
          </div>
        </div>
      </div>
