...
export default function Flight1GBookForm({ airportsData }: any) {
  ...
  const onSubmit = (data: FlightBookingInforType) => {
    ...
    flights.map((item, index) => {
      ...
      total_price_net += item.totalPrice;
      total_tax += item.totalTaxAdt + item.totalTaxChd + item.totalTaxInf;
      total_price += item.totalPrice;
      total_fee_service += item.totalServiceFee;
    });

    fare_data.push({
      session: flightSession,
      fare_data_id_api: flights?.[0]?.hpb_id,
      source: "1G",
      flights: flights,
    });
  ...
  flights.map((item, index: number) => {
    ...
    totalTaxAdt += item.taxAdult ?? item.totalTaxAdt;
    totalTaxChd += item.taxChild ?? item.totalTaxChd;
    totalTaxInf += item.taxInfant ?? item.totalTaxInf;
    totalPriceAdt += item.totalAdult;
    totalPriceChd += item.totalChild;
    totalPriceInf += item.totalInfant;
      ...
      <div className="flex flex-col-reverse items-start md:flex-row md:space-x-8 lg:mt-4 pb-8">
        ...
        <div className="w-full md:w-5/12 lg:w-4/12 bg-white rounded-2xl pb-0 ">
          ...
          <div>
            {flights.map((item, index) => {
              const flight =
                flightType !== "international" && isRoundTrip
                  ? item
